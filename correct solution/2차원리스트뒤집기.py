...
# zip함수를 씌우면 (1,4,7) (2,5,8) (3,6,9) 튜플 iterator가 리턴된다. zip()함수는 zip object를 리턴한다.
# map 함수를 이용해 각 튜플에 list함수를 적용시킨다. [1,4,7] [2,5,8] [3,6,9]의 iterator가 리턴된다. map()함수는 map object를 리턴한다. map object는 iterator이다(https://dojang.io/mod/page/view.php?id=2286)
# list함수로 iterator들을 list로 만든다. [[1,4,7], [2,5,8], [3,6,9]]

# 현재 정확히는 알 수 없지만, map(), zip() 함수 결과는 iterable이 아니라 iterator이다. 따라서 iterable로 만들기 위해 list()나 dict()등으로 확실하게 iterable로 만들어야한다.(https://dojang.io/mod/page/view.php?id=2405 의 그림 39-2)
